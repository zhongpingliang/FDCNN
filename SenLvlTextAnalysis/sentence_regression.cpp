...
SentenceRegression::SentenceRegression(){
	nor = 1;
}

SentenceRegression::SentenceRegression(const SentenceRegression & instance){}

void SentenceRegression::make_dict(){
	fprintf(stdout, "Start making dictionary\n");
	char buffer[MAX_WORD_LEN], ch = 0;
	integer i = 0, j = 0, nflds = 0;
	string fld;
	const integer ndicts = nif;
	integer ids[MAX_FIELD_NUM];
...
void SentenceRegression::make_dict_using_word2vec(){
	fprintf(stdout, "Start making dictionary\n");
	char buffer[MAX_WORD_LEN], ch = 0;
	integer i = 0, j = 0, nflds = 0;
	string fld;
...
void SentenceRegression::get_sentence_regression_example(FILE * fin, string * sen,	map<string, integer>::iterator **features,	real * label, integer &lst, long long &linu){
	char buffer[MAX_WORD_LEN], ch = 0;
	integer i = 0, j = 0, nflds = 0;
	string fld;
	map<string, integer>::iterator tmp;
	lst = 0;
	if (feof(fin)) return;
	else while (!feof(fin)) {
		ch = fgetc(fin);
	...
	do{
		...
		for (nflds = 0, i = 0; !feof(fin);){
			...
			if (ch == '\n') {
				...
				buffer[i] = 0;
				fld.assign(buffer, i);
				if ((tmp = dicts[nflds].find(fld)) == dicts[nflds].end()) tmp = dicts[nflds].find(NIL);
				features[nflds][lst] = tmp;
				++lst;
				break;
			}
			else if (ch == '\t'){
...
void SentenceRegression::load_model(){
...
void SentenceRegression::save_model(){
...
void SentenceRegression::save_model_binary(){
	fprintf(stdout, "Start saving model\n");
	...
	fwrite(U, sizeof(real), nhr[nhl - 1] + 1, model_file);
	fprintf(stdout, "Finish saving model\n");
}

void SentenceRegression::load_model_binary(){
	fprintf(stdout, "Start loading model\n");
	...
	fread(U, sizeof(real), nhr[nhl - 1] + 1, model_file);
	fprintf(stdout, "Finish loading model\n");
}

void SentenceRegression::train_sentence_regression_thread(integer thrd_id){
	...
	while (true) {
		mtx_tfi.lock();
		get_sentence_regression_example(train_fin, sen, features, &label, lst, linu);
		if (lst == 0){
			if (citer < iter_num){
				++citer;
				rewind(train_fin);
...
void SentenceRegression::predict_sentence_regression_thread(integer thrd_id){
	...
	while (true) {
		mtx_pfi.lock();
		get_sentence_regression_example(pred_fin, sen, features, &label, lst, linu);
		mtx_pfi.unlock();
		if (lst == 0) break;
		for (i = 0; i < ncl; ++i) ndk[i] = dynamic_k(lst, i);
		look_up_table(features, SENM, lst);
...
void SentenceRegression::train(integer argc, char **argv){
	if (argc == 1) {
	...
	for (i = 0; i < thrd_num; ++i){
		threads.push_back(thread(std::mem_fn(&SentenceRegression::train_sentence_regression_thread), this, i));
	}
	for (i = 0; i < thrd_num; ++i){
		threads.at(i).join();
	}
...
void SentenceRegression::predict(integer argc, char **argv){
	if (argc == 1) {
		fprintf(stdout, "Sentence classification toolkit v 0.1b\n\n");
	...
	vector<thread> threads;
	for (i = 0; i < thrd_num; ++i){
		threads.push_back(thread(std::mem_fn(&SentenceRegression::predict_sentence_regression_thread), this, i));
	}
	for (i = 0; i < thrd_num; ++i){
		threads.at(i).join();
	}
	fprintf(stdout, "Result : \n"
		...
		"Average prediction error = %lf\n", prec_exp_cnt, total_prec_err / static_cast<real>(prec_exp_cnt));
	fclose(pred_fin);
